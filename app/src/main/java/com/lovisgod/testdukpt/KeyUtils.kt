...
import com.lovisgod.testdukpt.Utils.byteArrayToHexString
import com.lovisgod.testdukpt.Utils.hexStringToByteArray
...
class KeyUtils {
    ...
    fun getWorkingKey(
        ...
        for (i in 0 until counterKSNbin.length) {
            ...
            sessionkey = BlackBoxLogicX().blackBoxLogic(newKSN2, initialIPEK)
            //Call Blackbox here
    ...
    fun DesEncryptDukpt(workingKey: String, pan: String, clearPin: String): String {
        val pinBlock = xorAndorOrFunction.XORorANDorORfunction(workingKey,
            BlackBoxLogicX().encryptPinBlock(pan, clearPin), "^")
